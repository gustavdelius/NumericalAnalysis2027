...
🖋 Show that for real $z$ the amplification factor $Q(z)=1+z+\frac{z^2}{2}$ of the midpoint method is always positive and that $|Q(z)|<1$ if and only if $-2<z<0$. Deduce that for real $\lambda<0$ the midpoint method is stable if and only if $h<2/|\lambda|$, the same condition as for Euler's method. Use this to explain the critical step size that you found for the midpoint method in part (d) of @exr-5.41.
:::
...
b.  Show that $|Q(z)|<1$ if and only if $\text{Re}(z)<0$, so that the region of absolute stability is exactly the left half-plane. Is the method unconditionally stable? (Hint: $|Q(z)|<1$ means that $z$ is closer to $-2$ than to $2$.)
...
b.  💻 Find numerically the interval of the negative real axis on which $|Q(z)|<1$, for example by using `np.roots()` to find the real roots of $Q(z)-1$ and $Q(z)+1$. This confirms the hint given in @exr-5.56. What is the resulting maximum step size for RK4 applied to $x'=-100x+\sin t$?
...
You should find that $|Q(z)|<1$ for $-2.785<z<0$, so the maximum step size is about $0.0279$.
:::
