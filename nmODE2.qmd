...
Setting $z = h\lambda$, the numerical solution decays to zero if and only if $|Q(z)| < 1$. Because the recurrence is linear, an error $\epsilon$ introduced at some step is also multiplied by $Q(h\lambda)$ at every subsequent step, so after $n$ further steps it has become $Q(h\lambda)^n\epsilon$. So errors decay if and only if $|Q(z)|<1$. We will see this argument again in @sec-stability-heat, where it is used to analyse the stability of the finite-difference method for the heat equation. The set of all $z \in \mathbb{C}$ for which $|Q(z)| < 1$ is called the **region of absolute stability** of the method. 
...
\end{cases}
$$
The error grows exponentially if $|1+h\lambda|>1$ and decays if $|1+h\lambda|<1$. So $|1+h\lambda|<1$ is the stability condition for the Euler method. For real negative $\lambda$ it requires us to choose a step size
$$
h<\frac{2}{|\lambda|}.
$$
:::
