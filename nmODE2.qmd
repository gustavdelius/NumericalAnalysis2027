...
Setting $z = h\lambda$, the numerical solution stays bounded if and only if $|Q(z)| \leq 1$ (and it decays to zero if and only if $|Q(z)| < 1$). Because the recurrence is linear, an error $\epsilon$ introduced at some step is also multiplied by $Q(h\lambda)$ at every subsequent step, so after $n$ further steps it has become $Q(h\lambda)^n\epsilon$. So errors do not grow if and only if $|Q(z)|\leq 1$. We will see this argument again in @sec-stability-heat, where it is used to analyse the stability of the finite-difference method for the heat equation. The set of all $z \in \mathbb{C}$ for which $|Q(z)| \leq 1$ is called the **region of absolute stability** of the method. 

Since we want the numerical solution not to grow whenever the exact solution decays, i.e.\ whenever $\text{Re}(\lambda) < 0$ (equivalently $\text{Re}(z) < 0$), we want the region of absolute stability to cover as much of the left half-plane as possible. Note that $\lambda$ may be complex, corresponding to oscillatory decaying solutions.
...
$$
We want to understand how errors evolve over time. If errors grow exponentially over time then we call the method unstable. Let us assume that at some step, for convenience let us choose step $0$, an error $\epsilon$ is introduced, for example by rounding:
$$
\tilde{x}_0=x_0+\epsilon.
$$
Then after $n$ steps we have
$$
\tilde{x}_n=(1+h\lambda)^n\tilde{x}_0=(1+h\lambda)^n(x_0+\epsilon)=(1+h\lambda)^nx_0+(1+h\lambda)^n\epsilon=x_n+(1+h\lambda)^n\epsilon.
$$
So the error at step $n$ is
$$
\epsilon_n=\tilde{x}_n-x_n=(1+h\lambda)^n\epsilon.
$$
Therefore
$$
\lim_{n\to\infty}|\epsilon_n|=\lim_{n\to\infty}|1+h\lambda|^n|\epsilon|
=\begin{cases}
0&\text{if }|1+h\lambda|<1\\
\infty&\text{if }|1+h\lambda|>1.
\end{cases}
$$
The error grows exponentially if $|1+h\lambda|>1$, stays the same size if $|1+h\lambda|=1$ and decays if $|1+h\lambda|<1$. So $|1+h\lambda|\leq 1$ is the stability condition for the Euler method. For real negative $\lambda$ it requires us to choose a step size
$$
h\leq\frac{2}{|\lambda|}.
$$
:::

So the amplification factor for the Euler method is $Q(z) = 1 + z$, and its region of absolute stability is the disk $|1 + z| \leq 1$, centred at $z = -1$ with radius $1$. For real negative $\lambda$ this recovers the step-size constraint $h \leq 2/|\lambda|$ derived above.
...
$$
For large $\gamma$ the term $e^{-\gamma t}$ is a rapidly decaying transient and the stiffness ratio is $\gamma/1=\gamma$. An explicit method must take $h\leq 2/\gamma$ to remain stable, even though the long-term behaviour of $x$ is governed by the much slower rate $e^{-t}$.
:::
