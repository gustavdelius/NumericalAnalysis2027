...
$$
 You know the values of $h=\Delta t$, $t_{n+1}$ and $x_n$, and you know the function $f$, so, in a practical sense, you should use some sort of numerical method to solve that equation -- at each time step. Since $G'(x_{n+1}) = 1 - h\,\partial f/\partial x$, applying Newton's method would require computing the partial derivative of $f$ with respect to $x$, which is not always available or convenient. So our next best option is to use the secant method. You can use your code from @exr-4.20 for the function `secant_1d()`.
...
The following plot lets you compare the backward Euler method to the forward Euler method and the exact solution. You just need to first define the `euler_1d()` function (which you have done in @exr-7.5) and the `backward_euler_1d()` function (which you have just completed in @exr-8.10).
...
x'=-100x+\sin t.
$$
This is similar to equations we solved in @exr-7.6, just with a larger negative coefficient in front of $x$. The solution to this equation is
$$
...
x(t)=Ce^{-100t}+\frac{100}{10001}\sin t-\frac{1}{10001}\cos t
$$
is indeed the general solution to $x'=-100x+\sin t$. (The particular solution can be found by the method of undetermined coefficients; the analytic solution to $x'=-\frac{1}{3}x+\sin t$ was stated in @exr-7.6.) Then:
...
$$
which describes a spring-mass system with damping. This is the system from @exr-7.9 with $m=1, b=\gamma+1$ and $k=\gamma$. Writing $u=x$ and $v=x'$ converts this to the first-order system
$$
...
a. Solve the system with the forward Euler method and step sizes $h=0.1$, $h=0.01$, and $h=0.001$. You can use your code from @exr-7.9. At which step size does forward Euler first produce a stable solution?
...
5.  Solve the system of differential equations from @exr-8.23 using `scipy.integrate.odeint()`. Show appropriate plots of your solution.
:::
