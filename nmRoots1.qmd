...
Note that the function does not just return the final approximation of the root but the list of all the midpoints that it computed along the way. The last element of that list is the final approximation. Returning all the iterates will allow us to study how quickly the method converges. We will do the same for all the other iterative methods in this module.
...
def bisection(f, a, b, tol=1e-5):
    ...
    Returns:
        list: the midpoints computed at each step. The last element
              is the approximate root of f(x)
    """
    ...
    # and start a list of iterates containing m
    ...
    # larger than 2 * tol
        ...
        # and append it to the list of iterates
...
Test your bisection method code on the following equations. Remember that your function returns the list of all midpoints, so the approximate root is the last element of that list, for example `bisection(f, a, b)[-1]`. For each equation make a plot of the function on the interval to verify your result.
...
💻 Now create a second version of your Python bisection method function that uses a `for` loop that takes the exact number of steps required to guarantee that the approximation to the root lies within a requested tolerance. Like your first version, it should return the list of all midpoints. This should be in contrast to your first version which likely used a `while` loop to decide when to stop. Is there an advantage to using one of these versions of the bisection method over the other?
...
Because our bisection function returns the list of all the midpoints, we can easily calculate the absolute error at each iteration. Here is a bisection function like the one you wrote in @exr-2.13:
```{python}
def bisection(f, a, b, tol=1e-5):
    """
    Find a root of f(x) in the interval [a, b] using the bisection method.
...
We can now use this function to calculate the vector of absolute errors at each iteration when solving the equation $x^2-2=0$ with the bisection method.
```{python}
import numpy as np
...
tolerance = 1e-7
iterates = bisection(f, 1, 2, tolerance)
# Convert the list of iterates to a NumPy array so that we can
# calculate all the errors at once
errors = np.abs(np.array(iterates) - x_exact)
errors
```
...
def plot_errors(errors):
    ...
    Parameters
        errors (NumPy array): Vector of absolute errors
    """
...
\ 2: \ \quad \textbf{for} \ n \gets 1 \ \textbf{to}\ N                         &\sharp \ tolerance \ tol, \ max. \ num. \ of \ iterations \ N \\
\ 3: \ \quad\quad x_n \ \gets g(x_{n-1})  & \\
\ 4: \ \quad\quad \textbf{if} \ |x_n-x_{n-1}| < tol \ \textbf{then}  &\sharp \ Desired \ tolerance \ reached  \\
\ 5: \ \quad\quad\quad \textbf{return} \ (x_0, x_1, \dots, x_n)  &\sharp \ all \ iterates \\
\ 6: \ \quad\quad \textbf{end \ if}  &  \\
\ 7: \ \quad \textbf{end for}  & \\
\ 8: \ \quad \textbf{exception}(Iteration \ has \ not \ converged) & \\
\ 9: \ \textbf{end function} &
\end{array}
$$

As for the bisection method, the function returns the list of all iterates $x_0, x_1, \dots, x_n$ rather than just the final approximation $x_n$.
...
def fixed_point_iteration(g, x0, tol, N):
    ...
    Returns:
        list: The iterates x_0, x_1, ..., x_n. The last element is the
              estimated fixed point if convergence is achieved within
              N iterations
    """
    x = [x0]
    for n in range(1, ???):
...
Fill in the missing parts of the code. Use the function to approximate the fixed point of the function $g(x)=\cos(x)$ with start value $x_0=2$ and tolerance $tol=10^{-8}$. The approximation is the last element of the returned list, which you can get with `[-1]`.
:::
...
(d) The equation $x^3 - x - 2 = 0$ can be rewritten as a fixed-point problem $x = g(x)$ in several ways. Consider the rearrangement $g(x) = \sqrt[3]{x+2}$. We have $g(1) \approx 1.44$ and $g(2) \approx 1.58$.
    ...
    ii) Complete the following Python code to perform 10 iterations of the fixed-point iteration $x_n = g(x_{n-1})$ starting with $x_0 = 1.0$ and to return the list of iterates $x_0, x_1, \dots, x_{10}$. [3 marks]
