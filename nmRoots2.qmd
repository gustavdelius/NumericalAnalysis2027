...
Root finding is really the process of finding the $x$-intercept of the function. If the function is complicated (e.g. highly nonlinear or does not lend itself to traditional by-hand techniques) then we can approximate the $x$-intercept by approximating the function by a truncated Taylor series (a Taylor polynomial, see @thm-taylor) at a nearby point and then finding the $x$-intercept of that simpler polynomial. The simplest non-trivial Taylor polynomial is a linear function: a tangent line.
...
::: proof
Applying Taylor's theorem (@thm-taylor) to $g$, centred at the point $p$ and with the Taylor polynomial of degree $m-1$, we obtain:
$$
\begin{split}
...
Near $x=x_0$ the function $f(x)$ is approximated by its Taylor polynomial of degree $N$ (see @thm-taylor)
$$
