...
::: {#exr-2.38}
💻 The following is an outline of a Python function called `newton()` for Newton's method. Complete the function definition by copying your code from your `fixed_point_iteration()` function and modifying it to implement Newton's method. Like `fixed_point_iteration()`, your `newton()` function should return the list of all iterates $x_0, x_1, \dots, x_n$.
...
def newton(f, fprime, x0, tol=1e-10, N=30):
    ...
    Returns:
        list: The iterates x_0, x_1, ..., x_n. The last element is the
              approximate root of f(x) if convergence is achieved within
              N iterations
    """
...
::: {#exr-2.39}
💻 🎓 Use your implementation from @exr-2.38 to approximate the root of the function $f(x) = x^2 - 2$ with an initial guess of $x_0=1$. Use a tolerance of $10^{-10}$. The approximation of the root is the last element of the list returned by your function.
:::
...
💻 🎓 Because your `newton()` function from @exr-2.38 returns the list of all iterates, you can calculate the vector of absolute errors between the iterates and the exact solution from that list, just as we did for the bisection method in @exm-2.1, with `np.abs(np.array(iterates) - x_exact)`.

Use this to list the error progression for Newton's method for finding the root of $f(x) = x^2 - 2$ with the initial guess $x_0 = 1$ and tolerance $10^{-10}$. You should find that the error goes down extremely quickly and that for the final iteration Python can no longer detect a difference between the new iterate and its own value for $\sqrt{2}$.
:::

::: {#exr-2.45}

1.  💻 Plot the errors you calculated in @exr-newton_error_tracking using the `plot_error_progression()` function that we wrote in @exm-2.1b. Note that you will first need to remove the zeros from the error vector, otherwise you will get an error when taking the logarithm. You can do this with `errors = errors[errors > 0]`, which keeps only the entries of the NumPy array `errors` that are greater than zero.
...
::: {#exr-2.49}
💻 🎓 Write a Python function for solving equations of the form $f(x) = 0$ with the secant method. Your function should accept a Python function, two starting points, and an optional error tolerance. It should return the list of all iterates $x_0, x_1, x_2, \dots, x_n$, including the two starting points, so that the last element is the approximation of the root. Also write a test script that clearly shows that your code is working.
:::
...
::: {#exr-2.50}
💻 Calculate the vector of absolute errors between the iterates returned by your secant function from @exr-2.49 and the exact solution, as we did for the bisection method in @exm-2.1.

Use this to list the error progression for the secant method for finding the root of $f(x) = x^2 - 2$ with the starting points $x_0 = 1$ and $x_1 = 3$. Use a tolerance of $10^{-10}$. Use the `plot_error_progression()` function that we wrote in @exm-2.1b to plot the error progression.
:::
...
3\sin(x) + 9 = x^2 - \cos(x).
$$
 Write a script that calls upon your bisection, Newton, and secant methods one at a time to find the positive solution to this equation. Remember that each of your functions returns the list of iterates, so the solution is the last element of that list. Your script needs to output the solutions in a clear and readable way so you can tell which answer came from which root-finding algorithm.
...
(a)  Solve for $x$ in the case that $N=2$. Then write a Python function that implements this root-finding method. As with your other root-finding functions, it should return the list of all iterates.
