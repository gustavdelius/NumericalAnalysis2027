...
💻 Complete the following function to implement this method. As with our root-finding functions, the function should return the list of all the successive middle points `c`, the last of which is the final approximation of the minimum. If you have done the previous two exercises you will have a good idea of what to do and this is mostly a coding task. So it would be alright to delegate this to just the group members who enjoy coding the most.
...
def golden_section(f, a, b, c, tol = 1e-12):
    ...
    Returns:
    list: The successive middle points `c`. The last element is an
          approximation of a point where f achieves a local minimum.
    """
    ...
    # Start a list of iterates containing `c`
    ...
    while b-a >= tol:
        ...
        # Append the new `c` to the list of iterates
...
::: {#exr-3.53}
💻 Write code to implement the 1D gradient descent algorithm. Note that gradient descent is a fixed-point iteration method. So you can adapt the code for Newton's method from @exr-2.38. Just be sure to change the iteration formula. As before, the function should return the list of all iterates.
...
def gradient_descent_1d(df, x0, alpha, 
    ...
    Returns:
    list: The iterates x_0, x_1, ..., x_n. The last element is an
          approximation of a point where f achieves a local minimum.
    """
    # Initialize the list of iterates with the starting value

    # Loop for a maximum of `max_iter` iterations

        # Calculate the next approximation and append it to the list
...
If you choose $x_0=3$ as your starting point and a learning rate of $0.001$, what approximation do you get for $x$ at the minimum (the last element of the list returned by your function)? Did gradient descent find the global minimum?
:::

------------------------------------------------------------------------

::: {#exr-3.55}
💻 🎓 Experiment with different values of the learning rate for the previous question, assuming that you can only specify it with up to 3 digits after the decimal point. Which choice of learning rate requires the smallest number of steps to reach the required tolerance of $10^{-12}$? You can get the number of steps from the length of the list of iterates returned by your function (remember that the list also contains the starting value).
...
1. Use your `gradient_descent_1d()` function from @exr-3.53 to calculate the list of iterates, and from that the vector of absolute errors between the iterates and the exact minimum, as you did for Newton's method in @exr-newton_error_tracking. 
2. Print the errors when applied to the function $f(x)=\cos(x)$ with a starting value of $x_0=3$, a learning rate of $\alpha = 0.1$ and a tolerance of $10^{-12}$. Make sure you use the correct exact minimum in your error calculations, namely the one that you expect gradient descent to find.
...
def gradient_descent(df, x0, alpha, tol=1e-12, max_iter=1000):
    ...
    Returns:
    list: The iterates x_0, x_1, ..., x_n, each a NumPy array with
          k elements. The last element is the approximated minimum point.
    """
...
The approximate minimum is the last element of the list of iterates returned by your function.
:::
...
1. Your `gradient_descent` function from @exr-3.58 returns the list of all iterates, which describes the path taken by the method. Use it to print the first few points on the path for the function in @exr-3.58 and check that each step goes downhill. How many iterations did the method need in that example?
...
SumSqRes = np.zeros(len(A)) # storage for the sums of the sq. residuals
for i, a in enumerate(A):
  guess = a*xdata**2 + b*xdata + c
  residuals = # write code to calculate the residuals
  SumSqRes[i] = ??? # calculate the sum of the squ. residuals
plt.plot(A, SumSqRes)
plt.grid()
