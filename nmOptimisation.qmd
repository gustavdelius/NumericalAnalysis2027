...
### Order of Convergence of Gradient Descent {#sec-gd-order}
...
The following exercises let you practise using the theory from this section.

::: {#exr-lect-gd-quadratic}
🖋 Consider the quadratic function $f(x)=\frac{c}{2}(x-p)^2$ with $c>0$, which has its minimum at $x=p$.

a.  Show that gradient descent with learning rate $\alpha$ gives $x_{n+1}-p=(1-\alpha c)(x_n-p)$ exactly, and hence $E_n=|1-\alpha c|^n E_0$.

b.  Describe the behaviour of the iterates for each of the cases $0<\alpha<\frac1c$, $\alpha=\frac1c$, $\frac1c<\alpha<\frac2c$, $\alpha=\frac2c$ and $\alpha>\frac2c$. In which cases do the iterates approach $p$ monotonically, in which cases do they oscillate around $p$, and in which cases do they fail to converge?

c.  Explain why, close to a minimum $p$ of a general smooth function $f$, gradient descent behaves like it does for the quadratic function with $c=f''(p)$. (Hint: Taylor-expand $f'(x)$ around $p$.)
:::

------------------------------------------------------------------------

::: {#exr-lect-gd-learning-rates}
🖋 Consider the function $f(x)=x^2-2\ln(x)$ for $x>0$.

a.  Show that $f$ has a minimum at $p=1$ and calculate $f''(1)$ and $f'''(1)$.

b.  For which learning rates $\alpha$ does gradient descent converge to $p=1$ when started sufficiently close to $p$?

c.  What is the limit $\lambda=\lim_{n\to\infty}E_{n+1}/E_n$ when using the learning rate $\alpha=0.1$?

d.  Which learning rate gives the fastest convergence? What is the order of convergence for this learning rate, and what is the corresponding constant $\lambda$? (Can you recognise the resulting iteration function $g(x)$ from @sec-roots2?)

You should find that the method converges for $0<\alpha<\frac12$, that $\lambda=0.6$ for $\alpha=0.1$, and that for $\alpha=\frac14$ the convergence is quadratic with $\lambda=\frac12$.
:::

------------------------------------------------------------------------

::: {#exr-lect-gd-cos}
🖋 💻 In @exr-3.54 you applied gradient descent to $f(x)=\cos(x)$ starting from $x_0=3$ with learning rate $\alpha=0.1$, so that the iterates converge to the minimum at $p=\pi$.

a.  Predict the value of $\lim_{n\to\infty}E_{n+1}/E_n$ and check it against your numerical errors from @exr-3.54.

b.  For which learning rates does gradient descent converge to $p=\pi$ (when started close enough)?

c.  Show that for the learning rate $\alpha=1$ not only $g'(\pi)=0$ but also $g''(\pi)=0$. What does @thm-a then tell you about the order of convergence? Check your prediction numerically by computing the ratios $E_{n+1}/E_n^3$.

You should find $\lambda=0.9$ in part (a), convergence for $0<\alpha<2$ in part (b), and cubic convergence with $\lambda=\frac16$ in part (c).
:::

------------------------------------------------------------------------

::: {#exr-lect-gd-maximum}
🖋 💬 Suppose $p$ is a local maximum of $f$ with $f''(p)<0$. Show that $|g'(p)|>1$ for the gradient descent iteration function $g(x)=x-\alpha f'(x)$ for every learning rate $\alpha>0$. What does this imply about whether gradient descent can converge to a local maximum? What happens to iterates that start close to $p$?
:::
...
::: {#exr-3.84}
🖋 💻 💬 The last step above is a bit vague. That was purposeful since there are many techniques that could be used to minimise the sum of the squared residuals. But the loss function $L(\boldsymbol{a})$ is just a function of the parameters, so we can minimise it with the tool that we built in @sec-multiopt: gradient descent. Let us do that for the quadratic $f(x) = ax^2 + bx + c$ and the data from @exr-3.81.

1.  🖋 Write the loss function as
$$
L(a,b,c) = \sum_{i=0}^{3} \left( y_i - (a x_i^2 + b x_i + c) \right)^2
$$
...
$$
Hint: use the chain rule. Each of the three partial derivatives is a sum over the data points of the residual $y_i - (a x_i^2 + b x_i + c)$ multiplied by something.

2.  💻 Complete the code below so that the function `dSSRes()` returns the gradient of the loss function. Go through the code and comment each line to describe exactly what it does.
...
def SSRes(parameters):
    """
    Sum of the squared residuals for the quadratic y = ax^2 + bx + c

    Parameters:
    parameters (NumPy array): The parameters a = parameters[0],
                              b = parameters[1] and c = parameters[2]
    ...
    yapprox = parameters[0]*xdata**2 + \
              parameters[1]*xdata + \
              parameters[2]
    residuals = ydata - yapprox
    return np.sum(residuals**2)

def dSSRes(parameters):
    """
    Gradient of the sum of the squared residuals with respect to
    ...
    Returns:
    NumPy array: The three partial derivatives of SSRes
    """
    yapprox = parameters[0]*xdata**2 + \
              parameters[1]*xdata + \
              parameters[2]
    residuals = ydata - yapprox
    gradient = np.zeros(3)
    gradient[0] = ??? # partial derivative with respect to a
    gradient[1] = ??? # partial derivative with respect to b
    gradient[2] = ??? # partial derivative with respect to c
    return gradient
```

3.  💻 Now use your `gradient_descent()` function from @exr-3.58 to find the parameters that minimise the loss function. Start from the parameters $(a,b,c) = (1,1,1)$ and use a learning rate of $\alpha = 0.008$, a tolerance of $10^{-8}$ and a maximum of $10\,000$ iterations. Then plot the best-fit quadratic together with the data.

``` python
best_parameters = gradient_descent(dSSRes, np.array([1.0, 1.0, 1.0]), 0.008,
                                   tol=1e-8, max_iter=10000)
print("The best values of a, b, and c are: \n", best_parameters)

plt.plot(xdata, ydata, 'bo')
x = np.linspace(0, 4, 100)
y = best_parameters[0]*x**2 + \
    best_parameters[1]*x + \
    best_parameters[2]
plt.plot(x, y, 'r--')
plt.grid()
...
You should find $a \approx 2.4225$, $b \approx -0.0665$ and $c \approx 2.2235$.

4.  💬 Now try the learning rate $\alpha = 0.01$. What happens? Try a few values in between. Relate what you observe to the condition on the learning rate that we derived for single-variable gradient descent in @sec-gd-order, see @eq-gd-rate. (For a function of several variables the role of $f''(p)$ is played by the largest eigenvalue of the matrix of second derivatives, which here is approximately $227$.)

5.  💬 How many iterations does gradient descent need to reach the tolerance with $\alpha = 0.008$? Compare this with the number of steps that gradient descent needed for the functions in @exr-gd-difficulties. Why do you think it needs so many steps here?
...
If you use gradient descent with the learning rate from @exr-3.84 on the 100 data points you will find that it diverges. You will need a learning rate that is more than ten times smaller, for example $\alpha = 0.0003$, and more iterations. Can you explain why the learning rate needs to be smaller when there are more data points?
:::
...
(e) We want to fit a straight line $f(x) = mx + c$ to the data points $(x_0,y_0), (x_1,y_1), \ldots, (x_n,y_n)$ using the method of least squares. Write down the loss function $L(m,c)$ that is minimised. [2 marks]

(f) Calculate the gradient $\nabla L(m,c)$ and show that setting it to zero leads to the pair of linear equations
$$
m \sum_{i=0}^n x_i^2 + c \sum_{i=0}^n x_i = \sum_{i=0}^n x_i y_i, \qquad
m \sum_{i=0}^n x_i + c\,(n+1) = \sum_{i=0}^n y_i.
$$
[3 marks]

(g) Use the equations from part (f) to find the least-squares line through the three data points $(0,1)$, $(1,2)$ and $(2,2)$. [3 marks]

(h) Instead of solving the equations from part (f), we could minimise $L(m,c)$ for the data in part (g) with gradient descent. Perform one step of gradient descent starting from $(m,c) = (0,0)$ with learning rate $\alpha = 0.1$. Does the loss decrease? [2 marks]
