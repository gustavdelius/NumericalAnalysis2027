...
def plot_path(func, path, x_range=(-2, 2), y_range=(-2, 2), grid_pts=100):
    ...
    Parameters:
        func (function): The function to plot. It should take a single
                         NumPy array of shape (2,) as its argument.
        path (list or array): The iterates of the optimisation method,
                              each a point (x, y).
        x_range (tuple): (x_min, x_max) for the surface (default: (-2, 2))
        y_range (tuple): (y_min, y_max) for the surface (default: (-2, 2))
        grid_pts (int): Number of grid points in each direction
                        (default: 100)

    Returns:
        plotly Figure: The figure showing the surface and the path.
    """
    path = np.array(path)
...
def SSRes(parameters):
  """Sum of the squared residuals of the quadratic fit to the data."""
...
def gradient_descent_1d(df, x0, alpha, tol=1e-12, max_iter=10000):
    """Find a local minimum of f using gradient descent, given f' = df."""
    x = x0
