...
sum_sq_res = [] # this is storage for the sum of the sq. residuals
for a in A:
  guess = a*xdata**2 + b*xdata + c
  residuals = # write code to calculate the residuals
  sum_sq_res.append( ??? ) # calculate the sum of the squ. residuals
plt.plot(A, sum_sq_res)
plt.grid()
...
best_parameters = minimize(ss_res,[1,1,1])
...
x = np.linspace(0,4,100)
y = best_parameters.x[0]*x**2 + \
    best_parameters.x[1]*x + \
    best_parameters.x[2]
plt.plot(x,y,'r--')
plt.grid()
plt.xlabel('x')
