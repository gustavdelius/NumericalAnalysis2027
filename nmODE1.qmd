...
🖋 💻 If a mass is hanging from a spring then Newton's second law, $\sum F=ma$, gives us the differential equation $mx'' = F_{restoring} + F_{damping}$ where $x$ is the displacement of the mass from equilibrium, $m$ is the mass of the object hanging from the spring, $F_{restoring}$ is the force pulling the mass back to equilibrium, and $F_{damping}$ is the force due to friction or air resistance that slows the mass down. The restoring force is proportional to the displacement away from equilibrium, $F_{restoring} = -kx$, where $k$ is the spring constant, and the damping force is proportional to the velocity of the mass, $F_{damping} = -bx'$, where $b$ is the damping coefficient. So the position of the mass satisfies the second-order differential equation
\begin{equation}
 m x'' = -b x' - k x.
\end{equation}

1. 🎓 If we want to solve a second order differential equation numerically we need to convert it to first order differential equations (Euler's method is only designed to deal with first order differential equations, not second order). To do so we introduce the two variables $u = x$ (the position) and $v = x'$ (the velocity). The result is a system of first-order differential equations for $u$ and $v$.
\begin{equation}
\begin{aligned} u' &= v \\ v' &= \underline{\hspace{2in}}\end{aligned}
\end{equation}


2.  The code and Euler's method algorithm that we have created thus far in this chapter are only designed to work with a single differential equation instead of a system, so we need to make some modifications. We can discretize the system of differential equations using Euler's method so that
\begin{equation}
...
\begin{equation}
 F(t,\boldsymbol{x}) = \begin{pmatrix} u' \\ v' \end{pmatrix} = \begin{pmatrix} v \\ \underline{\hspace{1in}} \end{pmatrix} \quad \text{where} \quad \boldsymbol{x} = \begin{pmatrix} u \\ v \end{pmatrix}.
\end{equation}


3.  We now need to discretize the derivatives in the system. As with 1D Euler's method, we will use a first-order approximation of the first derivative so that
\begin{equation}
...
4.  The following Python function will implement Euler's method. Complete the code.
...
5.  To use the `euler()` function to solve the system of equations from part (1), complete the following code. Use a mass of $m=2$kg, a damping force of $b=40$kg/s, and a spring constant of $k=128$N/m. Consider an initial position of $u = 0$ (equilibrium) and an initial velocity of $v = 0.6$m/s. Note that inside the function `F` the vector `x` holds the two variables, `x[0]` $= u$ and `x[1]` $= v$.
...
k = 128
def F(t, x):
    u, v = x  # unpack the vector x into position and velocity
    return np.array([v, ???])
...
6. 🎓 We will want to make the same kind of plots for many different systems, so it is a good idea to wrap the plotting code into a function. Complete the following function that makes a plot showing both components of the solution, here position and velocity, versus time.

``` python
def plot_time_evolution(t, x, labels=('position', 'velocity')):
    """
    Plot both components of the solution of a system of two
    first-order ODEs against time

    Parameters:
    t (NumPy array): Vector of time values
    x (NumPy array): Array of solution values with one row for each
                     time and one column for each dependent variable
    labels (tuple): Names of the two dependent variables, used in
                   the legend
    """
...
7. Complete the following function to make a second plot, called a phase plot, that shows position versus velocity. In a phase plot, time is implicit (not one of the axes).

``` python
def plot_phase(x, labels=('position', 'velocity')):
    """
    Make a phase plot of the solution of a system of two first-order
    ODEs, i.e., plot the second component against the first

    Parameters:
    x (NumPy array): Array of solution values with one row for each
                     time and one column for each dependent variable
    labels (tuple): Names of the two dependent variables, used as
                   axis labels
    """
    plt.plot(x[???, ???], x[???, ???])
    plt.grid()
...
3.  Create the time evolution plot with your `plot_time_evolution()` function from @exr-5.17, using `labels=('rabbits', 'foxes')`. What does this plot tell you in context?

4.  Create a phase plot with your `plot_phase()` function. What does this plot tell you in context?
