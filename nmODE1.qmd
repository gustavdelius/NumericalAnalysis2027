...
::: {#def-5.4}
#### Euler's Method
Euler's Method is a technique for approximating the solution to the differential equation $x'(t) = f(t,x(t))$. Recall from @exr-5.9 that the first derivative of a function can be discretized as
\begin{equation}
...
::: {#exr-7.12}
🖋 💬 Let us return to the simple differential equation $x' = -0.5x$ with $x(0) = 6$ that we saw in @exr-7.1. Now we will propose a slightly different method for approximating the solution.
...
d.  How does your approximation compare to the Euler approximation that you found in @exr-7.1?
...
::: {#exr-7.13}
🖋 🎓 As in in @exr-7.4, consider the differential equation $x'(t) = -2x(t)/3 + 4t$ with initial condition $x(0)=6$. By hand perform one step of the midpoint method with stepsize $h=1$ to obtain an approximation for $x(1)$.
...
::: {#exr-7.16}
💻 🎓 Repeat @exr-7.7 with the midpoint method. Compare your results to what you found with Euler's method.
Then use what you have discovered to answer the following, similar to @exr-7.8:
