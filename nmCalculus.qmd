...
::: {#exr-3.17}
💻 🎓 We want to build a Python function `ForwardDiff(f, a, b, N)` that accepts:
...
3.  Use the insight from part (2) to complete the code below and update the comments to say what each line does.
...
def ForwardDiff(f,a,b,N):
    ...
    y = f(x) # What does this line of code do?
    df = # your line of code goes here
    return df
```

4.  Now we want to call upon this function to build the first-order approximation of the first derivative for some function. We will use the function $f(x) = \sin(x)$ on the interval $[0,2\pi]$ with 100 subintervals (since we know what the answer should be). Complete the code below to call upon your `ForwardDiff()` function and to plot $f(x)$, $f'(x)$, and the approximation of $f'(x)$.
...
5.  Implement your completed code and then test it in several ways:
...
The following exercises let you practise the truncation-error analysis from this section.

::: {#exr-lect-trunc-left-riemann}
🖋 Carry out the analysis that we did above for the right Riemann sum for the **left** Riemann sum, which uses the approximation $A_j=f(x_{j-1})h$ on the subinterval $[x_{j-1},x_j]$.

a.  Show that $I_j-A_j=f'(\xi_j)\frac{h^2}{2}$ for some $\xi_j$ between $x_{j-1}$ and $x_j$, and hence that the local truncation error is $\tau_j=\frac{h}{2}|f'(\xi_j)|$.

b.  Show that the global truncation error satisfies $\tau\leq\frac{b-a}{2}hM$ where $M=\max_{a\le x\le b}|f'(x)|$.

c.  Compare the sign of $I_j-A_j$ with the sign we found for the right Riemann sum. The trapezoidal rule is the average of the left and the right Riemann sums. Use this to explain why the first-order error terms cancel in the trapezoidal rule, so that it is of higher order than either Riemann sum.
:::

------------------------------------------------------------------------

::: {#exr-lect-trunc-simpson-combination}
🖋 Let $M_j=hf(m_j)$ denote the midpoint approximation and $T_j=\frac{h}{2}\left(f(x_{j-1})+f(x_j)\right)$ the trapezoidal approximation to the integral $I_j$ over the subinterval $[x_{j-1},x_j]$.

a.  Show that Simpson's rule is a weighted average of these two: $A_j=\frac{2}{3}M_j+\frac{1}{3}T_j$.

b.  Use the leading-order error terms derived above, $I_j-M_j\approx \frac{h^3}{24}f''(m_j)$ and $I_j-T_j\approx -\frac{h^3}{12}f''(m_j)$, to explain why exactly these weights $\frac23$ and $\frac13$ make the $h^3$ error terms cancel.

c.  Use the error formulas from this section to explain why the midpoint rule and the trapezoidal rule integrate all polynomials of degree at most $1$ exactly, and why Simpson's rule integrates all polynomials of degree at most $3$ exactly (even though it is built from parabolas).
:::

------------------------------------------------------------------------

::: {#exr-lect-trunc-choose-N}
🖋 We want to approximate $\int_0^1 e^x\,dx$ with an error of at most $10^{-6}$.

a.  By summing the local errors over all $N$ subintervals, just as we did for the right Riemann sum, show that the global truncation error of the trapezoidal rule satisfies
$$
\tau\leq \frac{b-a}{12}h^2M_2,\qquad M_2=\max_{a\le x\le b}|f''(x)|.
$$

b.  Use this bound to determine how many subintervals $N$ are needed for the trapezoidal rule to guarantee an error of at most $10^{-6}$ for this integral.

c.  Derive the analogous bound $\tau\leq\frac{b-a}{2880}h^4M_4$ for Simpson's rule (in the form used in this section, where each of the $N$ subintervals of width $h$ uses its two endpoints and its midpoint) and use it to determine the number of subintervals needed for Simpson's rule.

You should find that $N=476$ subintervals are needed for the trapezoidal rule but only $N=6$ for Simpson's rule.
:::

------------------------------------------------------------------------

::: {#exr-lect-trunc-choose-N-check}
💻 Use your trapezoidal-rule and Simpson's-rule functions to check the claims of the previous exercise. Compute the actual absolute errors for $\int_0^1 e^x\,dx=e-1$ with the trapezoidal rule for $N=475$ and $N=476$ and with Simpson's rule for $N=5$ and $N=6$. Compare them with the error bounds. Why is the actual error smaller than the bound? (Hint: in the bound we replaced $|f''(\xi_j)|$ by its maximum value on the whole interval.) Is $N=5$ perhaps already good enough for Simpson's rule?
:::
