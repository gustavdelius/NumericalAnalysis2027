...
def plot_forward_difference_errors(f, x, exact, H):
    ...
    # Create vector to hold the errors
    AbsError = np.zeros(len(H))
    for i, h in enumerate(H):
        approx = (f(x+h) - f(x))/h
        AbsError[i] = abs(approx - exact)
...
def plot_riemann_errors(f, a, b, exact, DX):
    ...
    # Create vectors to hold the errors
    error_left = np.zeros(len(DX))
    error_right = np.zeros(len(DX))
    error_midpoint = np.zeros(len(DX))

    for i, dx in enumerate(DX):
        N = int(np.round((b - a) / dx))
        error_left[i] = abs(RiemannSum(f, a, b, N, method='left') - exact)
        error_right[i] = abs(RiemannSum(f, a, b, N, method='right') - exact)
        error_midpoint[i] = abs(RiemannSum(f, a, b, N, method='midpoint') - exact)
