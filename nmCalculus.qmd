...
def plot_forward_difference_errors(f, x, exact, H):
    """
    Plot the absolute error in the forward-difference approximation of f'(x).

    Produces a log-log plot with the step sizes on the x axis and the
...
def ForwardDiff(f,a,b,N):
    """
    Approximate the first derivative of f on [a,b] using forward differences.
    ...
    Returns:
        list: Approximations of f'(x) at all N+1 equally spaced points
              in [a,b] except the right endpoint
    """
...
def ForwardDiff(f,a,b,N):
    ...
    Returns:
        NumPy array: Approximations of f'(x) at all N+1 equally spaced
                     points in [a,b] except the right endpoint
    """
    x = np.linspace(a,b,N+1)
    h = x[1] - x[0]
    y = f(x)
...
def plot_max_forward_difference_errors(f, f_prime, a, b, H):
    ...
    Produces a log-log plot with the step sizes on the x axis and the
    maximum absolute error on the y axis.
...
def plot_riemann_errors(f, a, b, exact, DX):
    """
    Plot the absolute error in the left, right and midpoint Riemann sum
    approximations.
