...
Throughout this chapter we will make heavy use of Taylor's theorem (@thm-taylor) to build approximations of derivatives and integrals. This has become a recurring scheme in this module: Taylor's theorem is your all-purpose approximation tool. It suggests the approximation methods and it allows you to understand the error that comes with the approximation.
...
::: {#exr-3.10}
🖋 Recall from @def-1.3 that, for a function $f(x)$ that is analytic at $x_0$ (so that its Taylor series represents it near $x_0$),
$$
...
::: {#exr-3.20}
🖋 Consider again the Taylor series of a function $f(x)$ that is analytic at $x_0$:
$$
 f(x) = f(x_0) + \frac{f'(x_0)}{1!} (x-x_0)^1 + \frac{f''(x_0)}{2!}(x-x_0)^2 + \frac{f^{(3)}(x_0)}{3!}(x-x_0)^3 + \cdots .
$$
...
Let us recall Taylor's theorem (@thm-taylor) from @sec-truncation_error: for a function $f$ that is $n+1$ times differentiable on an interval containing $x$ and $x_0$,
$$
\begin{split}
...
$$
for some $\xi$ between $x_0$ and $x$. The error introduced by truncating the series after the term of degree $n$ is thus $|R_n(x)|$. In the step-size form, with $x_0$ replaced by $x$ and $x$ replaced by $x+h$,
$$
f(x+h)=f(x)+f'(x)\,h+\cdots+\frac{f^{(n)}(x)}{n!}h^n+\frac{f^{(n+1)}(\xi)}{(n+1)!}h^{n+1}
$$
...
\begin{split}
f'(x)&\approx \frac{f(x+h)-f(x)}{h}\\
&=\frac{f(x)+hf'(x)+\frac{h^2}{2}f''(\xi)-f(x)}{h}\\
&=f'(x)+\frac{h}{2}f''(\xi).
\end{split}
$$
for some $\xi$ between $x$ and $x+h$, where we used Taylor's theorem with $n=1$. We see that the truncation error is proportional to the step size $h$. We say that the truncation error is of order $h$ and that this forward-difference approximation is a first-order approximation. This means that in order to halve the size of the error we need to halve the step size.
...
&=\int_{x_{j-1}}^{x_{j}}\left(f(x_j)+f'(\xi_x)(x-x_j)-f(x_j)\right)dx\\
&=f'(\xi_j)\int_{x_{j-1}}^{x_{j}}(x-x_j)dx\\
&=-f'(\xi_j)\frac{h^2}{2}\\
\end{split}
$$
for some $\xi_j$ between $x_{j-1}$ and $x_j$. Here we first used Taylor's theorem with $n=0$ (which is just the mean value theorem), centred at $x_j$, where $\xi_x$ lies between $x$ and $x_j$ and depends on $x$. In the next step we used the mean value theorem for integrals to take $f'$ out of the integral, which is allowed because $(x-x_j)$ does not change sign on the interval. So the local truncation error is
$$
...
\end{split}
$$
for some $\xi_j$ between $x_{j-1}$ and $x_j$. Here we used Taylor's theorem with $n=1$, centred at $m_j$, with $\xi_x$ between $x$ and $m_j$, and then, as for the right Riemann sum, the mean value theorem for integrals (using that $(x-m_j)^2\geq 0$). Since $\int_{x_{j-1}}^{x_{j}}(x-m_j)dx=0$ and $\int_{x_{j-1}}^{x_{j}}(x-m_j)^2dx=\frac{h^3}{12}$, we have
$$
