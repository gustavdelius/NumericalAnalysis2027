...
def plot_solution_1d(t, x, U):
    ...
    Parameters:
        ...
        x (NumPy array): A vector of spatial values.
        U (NumPy array): A 2D array approximating the solution u(t,x)
            at each grid point.

    Returns:
        plotly Figure: A 3D surface plot of the solution.
    """
...
def animate_solution_1d(t, x, U):
    ...
    Returns:
        FuncAnimation: An animation of u against x as t evolves.
    """
    fig, ax = plt.subplots()
    plt.close()
    ax.grid()
...
def heat1d(u_0, D=0.1, t_0=0, t_max=1, N_t=100, x_left=0, x_right=1, N_x=20):
    ...
    Parameters:
        u_0 (function): A function giving the initial condition u(0,x).
        D (float): The diffusion coefficient. Defaults to 0.1.
        t_0 (float): The initial time. Defaults to 0.
        t_max (float): The maximum time. Defaults to 1.
        N_t (int): The number of time steps. Defaults to 100.
        x_left (float): The left boundary of the spatial domain.
            Defaults to 0.
        x_right (float): The right boundary of the spatial domain.
            Defaults to 1.
        N_x (int): The number of spatial steps. Defaults to 20.
...
def solve_heat1d(u_0, D, tmax, dt, xmax, dx):
    """
    Solves the 1D heat equation u_t = D u_xx on [0, xmax] with
    u = 0 at both boundaries, using the explicit scheme.
    ...
    Parameters:
        ...
        xmax (float): The right boundary of the spatial domain.
